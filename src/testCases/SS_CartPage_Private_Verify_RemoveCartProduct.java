...
/**
 * 
 * <h2 style="text-align:center;">SS_CartPage_Private_Verify_RemoveCartProduct</h2>
 * <p style="font-size:19px"><b>Description -</b>This Test Case verifies removing product from mini cart functionality for a registered user.</p>
...
public class SS_CartPage_Private_Verify_RemoveCartProduct {
	...
	public void main() throws Exception {
		try {
			...
			Cart_Action.CheckMiniCartRemoveFunctionality(iTestCaseRow);
			ExcelUtils.setCellData("Pass", iTestCaseRow, Constant.result);
			Log.info("Verification for removing product in minicart for registered user successfull");
			Utils.captureScreenshot(sTestCaseName, "Pass", "Cart Page Remove Cart for Registered User");
		} catch (Exception e) {
			Log.info("Verification for removing product in minicart for registered user failed");
